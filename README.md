...
HTML `TEST GUIDES` include `all`, `no`, and `best`. In `DETAIL`, guide lines cannot be hidden; clicking a guide chip jumps to the next matching specification and cycles when multiple matches exist. In `COMPACT`, guide chips keep the old show/hide behavior. `best` is a yellow helper line shown when the 3-star significant specifications contain no full-test specification; it marks the 3-star specification with the largest number of included `controls_test` variables, breaking ties by smaller p-value and then original order.
...
The right-side detail panel always shows content in `DETAIL` mode, even before any specification is hovered or pinned. With no active specification, statistic values stay empty while configured control rows remain visible. It always shows all `controls_test` variables in the original input order. Variables not included in the current specification keep their name and a blank badge box, while coefficient and p-value cells stay empty.
...
In the right-side detail panel, clicking a control-variable row toggles a control-significance filter. Multiple selected rows are combined with AND semantics, so the main chart removes non-matching specifications and re-packs the remaining columns so the kept specifications are contiguous; every kept specification has each selected control significant at the 10% level or better. Active filters remain visible in the sidebar and can be cleared there.
