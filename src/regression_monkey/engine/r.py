...
def _write_fixest_script(
    *,
    ...
    lines: list[str] = [
        ...
        "  } else {",
        "    fml <- stats::as.formula(paste0(qvar(y_var), ' ~ ', rhs))",
        "  }",
        "  model <- tryCatch({",
        "    if (se_kind == 'robust') {",
        ...
        *([
        ...
        "    dm <- as.matrix(dm)",
        "  } else {",
        "    dm <- as.matrix(work[, demean_vars, drop = FALSE])",
        "  }",
        ...
        "  if (!length(fe_terms)) X_raw <- cbind('(Intercept)' = 1, X_raw)",
        "  keep <- keep_independent(X_raw)",
        "  x_raw_idx <- match(x_var, colnames(X_raw))",
        "  if (!length(keep) || is.na(x_raw_idx) || !(x_raw_idx %in% keep)) return(NULL)",
        ...
        "  sse <- sum(e^2)",
        "  tss <- if (length(fe_terms)) sum(y_dm^2) else sum((y_dm - mean(y_dm))^2)",
        ...
        "  if (!is.null(vcov_inv)) {",
        "    test_vars <- kept_vars[kept_vars != '(Intercept)']",
        "    if (length(test_vars)) {",
        "      beta <- as.numeric(coef_full[test_vars])",
        "      sub_vcov_inv <- tryCatch(solve(vcov_mat[test_vars, test_vars, drop = FALSE]), error = function(err) NULL)",
        "      if (!is.null(sub_vcov_inv)) f_stat <- as.numeric(t(beta) %*% sub_vcov_inv %*% beta / length(beta))",
        "    }",
        "  }",
        "  ctrl_stats <- list()",
        ...
        "    }",
        "  }",
        "  controls_all <- kept_vars[!(kept_vars %in% c(x_var, '(Intercept)'))]",
